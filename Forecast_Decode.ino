...
/***************************************************************************/
{
  ...
  if ((WiFi.status() == WL_CONNECTED)) //Check the current connection status
  {
    ...
    Serial.print("\r\nOWM Forecast Request at "); Serial.println(fullTimeDate);
    Serial.print(ForecastEndpoint);
    ...
    if (httpCode > 0) {  //Check for the returning code
      ...
      Serial.println("OWM Return Packet");
      Serial.println(payload);
    }
    else {
      Serial.printf("Error %i on HTTP request. Retrying once.\r\n", httpCode);
      http.begin(ForecastEndpoint); //Specify the URL
      httpCode = http.GET();      //Make the request
      if (httpCode > 0) {  //Check for the returning code
        payload = http.getString();
        ...
        Serial.printf("Error %i on HTTP request. exiting\r\n", httpCode);
        return;
      }
      http.end(); //Free the resources
    }
    dataFetchHour = intHour; dataFetchMin = intMin;
    payload.toCharArray(inp, 50000);

    error = deserializeJson(doc, inp);
    if (error) {
